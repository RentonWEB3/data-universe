...
def parse_desirability_json(json_file: str) -> DataDesirabilityLookup:
...
async def run_retrieval(config) -> DataDesirabilityLookup:
    try:
        ...
        return parse_desirability_json(os.path.join(script_dir, AGGREGATE_JSON_PATH))

    except Exception as e:
        bt.logging.error(f"Could not retrieve dynamic preferences. Using default.json to build lookup: {str(e)}")
        script_dir = os.path.dirname(os.path.abspath(__file__))
        return parse_desirability_json(os.path.join(script_dir, DEFAULT_JSON_PATH))
