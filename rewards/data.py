...
class DataDesirabilityLookup(StrictBaseModel):
    ...
    def validate_distribution(
        ...
        if not abs(sum(d.weight for d in distribution.values()) - 1.0) < 1e-6:
    ...
    def get_weight(self, entity: "DataEntity") -> float:

        """

        Вычисляет desirability вес для переданной сущности.
        ...
        source_desirability = self.distribution.get(entity.source)

        if not source_desirability:

            return 0.0



        if not isinstance(entity.datetime, datetime):

            return 0.0



        age_hours = (datetime.now(timezone.utc) - entity.datetime).total_seconds() / 3600

        if age_hours > self.max_age_in_hours:
        ...
        if entity.label is not None:
            ...
            scale = source_desirability.label_scale_factors.get(

                entity.label, source_desirability.default_scale_factor

            )

        else:

            scale = source_desirability.default_scale_factor



        return source_desirability.weight * scale
